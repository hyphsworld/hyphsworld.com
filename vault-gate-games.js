(() => {
  ...
  function spinNeonSlots() {
    if (slotSpinning) return;
    slotSpinning = true;
    const panel = $('.neon-slots-panel');
    const reels = ['neonReel1', 'neonReel2', 'neonReel3'].map((id) => $('#' + id)).filter(Boolean);
    const spinBtn = $('#neonSpinBtn');
    const slotStatus = $('#neonSlotStatus');
    const icons = ['💚', '💡', '❇️', '💎', '🎰', '🟢', '⚡', '🔊'];
    const finalIcons = reels.map(() => icons[Math.floor(Math.random() * icons.length)]);
    if (spinBtn) spinBtn.disabled = true;
    if (panel) panel.classList.add('is-spinning');
    reels.forEach((reel) => reel.classList.add('is-spinning'));
    if (slotStatus) slotStatus.textContent = 'Neon reels spinning...';
    let ticks = 0;
    const timer = window.setInterval(() => {
      ticks += 1;
      reels.forEach((reel) => { reel.textContent = icons[Math.floor(Math.random() * icons.length)]; });
      if (ticks >= 18) {
        window.clearInterval(timer);
        reels.forEach((reel, index) => {
          window.setTimeout(() => {
            reel.textContent = finalIcons[index];
            reel.classList.remove('is-spinning');
            if (index === reels.length - 1) {
              if (panel) panel.classList.remove('is-spinning');
              const jackpot = finalIcons.every((icon) => icon === finalIcons[0]);
              const reward = jackpot ? 25 : 5;
              addPoints(reward, jackpot ? 'neon_slots_jackpot' : 'neon_slots_spin');
              setPlayerStatus(jackpot ? 'NEON JACKPOT counted for Cool Points.' : 'Neon slot spin counted for Cool Points.');
              if (slotStatus) slotStatus.textContent = jackpot ? '+25 Cool Points. Neon jackpot.' : '+5 Cool Points. Neon light spin logged.';
