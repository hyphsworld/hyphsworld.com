(function (global) {
  ...
  async function upsertRow(user, updates = {}) {
    ...
    const row = { id: user.id, username: String(updates.username || current?.username || meta.username || usernameFromEmail(user.email)).slice(0, 30), display_name: String(updates.displayName || updates.display_name || current?.display_name || meta.displayName || displayFromEmail(user.email)).slice(0, 40), duck_status: String(updates.duckStatus || updates.duck_status || current?.duck_status || meta.duckStatus || 'Duck Sauce is watching this account.').slice(0, 90), buck_clearance: String(updates.buckClearance || updates.buck_clearance || current?.buck_clearance || meta.buckClearance || 'Lobby clearance only').slice(0, 90), avatar_type: nextAvatarType, avatar_icon: avatarIcon(nextAvatarType), points: nextPoints, lifetime_points: nextLifetime, level_1_unlocked: Boolean(current?.level_1_unlocked || updates.level1Unlocked || updates.level_1_unlocked), level_2_unlocked: Boolean(current?.level_2_unlocked || updates.level2Unlocked || updates.level_2_unlocked), updated_at: new Date().toISOString() };
  ...
  async function addLedger(user, amount, reason) { const sb = await getClient(); if (!sb || !user || !amount) return; try { await sb.from('points_ledger').insert({ user_id: user.id, amount, reason: reason || 'site_action', metadata: { source: 'hyphsworld_frontend' } }); } catch (error) {} }
  async function addVaultUnlock(user, levelKey) { const sb = await getClient(); if (!sb || !user) return; try { await sb.from('vault_unlocks').insert({ user_id: user.id, level_key: levelKey || 'level_1' }); } catch (error) {} }
  ...
  async function signUpWithEmail(email, password) {
    ...
    const { data, error } = await sb.auth.signUp({ email, password, options: { emailRedirectTo: AUTH_REDIRECT_URL, data: { displayName, username: usernameFromEmail(email), duckStatus: 'Duck Sauce has not fined this account yet.', buckClearance: 'Lobby clearance only', avatarType: 'boy', avatarIcon: '🧢', points: startingPoints, lifetimePoints: startingPoints } } });
    ...
    saveLocalSession(session); saveLocalProfileName(displayName); saveLocalAvatar('boy'); return session;
  }
  ...
  async function signInWithEmail(email, password) {
    ...
    saveLocalPoints(row?.points ?? 0); saveLocalProfileName(row?.display_name || displayFromEmail(email)); saveLocalAvatar(row?.avatar_type || 'boy');
  ...
  async function getSession() { const sb = await getClient(); if (!sb) return localSession(); const { data } = await sb.auth.getSession(); if (!data || !data.session || !data.session.user) { clearLocalSession(); return null; } const session = sessionFromUser(data.session.user); saveLocalSession(session); return session; }

  async function getCurrentUser() {
    const sb = await getClient();
    if (sb) {
      const { data } = await sb.auth.getUser(); if (!data || !data.user) return null;
      const user = data.user; const row = (await rowFor(user)) || (await upsertRow(user, { displayName: user.user_metadata?.displayName || displayFromEmail(user.email) }));
      const points = num(row?.points ?? user.user_metadata?.points ?? 0); const type = avatarType(row?.avatar_type || user.user_metadata?.avatarType || 'boy');
      saveLocalPoints(points); saveLocalProfileName(row?.display_name || user.user_metadata?.displayName || displayFromEmail(user.email)); saveLocalAvatar(type);
      return { email: user.email || '', userId: user.id || '', provider: 'supabase', displayName: row?.display_name || user.user_metadata?.displayName || displayFromEmail(user.email), username: row?.username || user.user_metadata?.username || usernameFromEmail(user.email), duckStatus: row?.duck_status || user.user_metadata?.duckStatus || 'Duck Sauce is watching this account from a folding chair.', buckClearance: row?.buck_clearance || user.user_metadata?.buckClearance || 'Lobby clearance only', avatarType: type, avatarIcon: row?.avatar_icon || avatarIcon(type), coolPoints: points, lifetimePoints: num(row?.lifetime_points ?? points), level1Unlocked: Boolean(row?.level_1_unlocked), level2Unlocked: Boolean(row?.level_2_unlocked) };
    }
    const session = localSession(); if (!session || !session.email) return null;
    const users = localUsers(); const stored = users[session.email] || {}; const type = avatarType(stored.avatarType || 'boy');
  ...
  async function updateProfile(updates) {
    const sb = await getClient(); const type = avatarType(updates.avatarType || updates.avatar_type || 'boy');
    if (sb) { const { data } = await sb.auth.getUser(); if (!data || !data.user) throw new Error('Login required.'); const current = await rowFor(data.user); const clean = { displayName: String(updates.displayName || current?.display_name || displayFromEmail(data.user.email)).trim().slice(0, 40), duckStatus: String(updates.duckStatus || current?.duck_status || 'Duck Sauce has no official notes.').trim().slice(0, 90), buckClearance: String(updates.buckClearance || current?.buck_clearance || 'Lobby clearance only').trim().slice(0, 90), avatarType: type, avatarIcon: avatarIcon(type) }; await sb.auth.updateUser({ data: clean }); await upsertRow(data.user, clean); saveLocalProfileName(clean.displayName); saveLocalAvatar(type); return getCurrentUser(); }
    const session = localSession(); if (!session || !session.email) throw new Error('Login required.'); const users = localUsers(); const current = users[session.email] || { email: session.email }; users[session.email] = { ...current, displayName: String(updates.displayName || current.displayName || displayFromEmail(session.email)).trim().slice(0, 40), duckStatus: String(updates.duckStatus || current.duckStatus || 'Duck Sauce has no official notes.').trim().slice(0, 90), buckClearance: String(updates.buckClearance || current.buckClearance || 'Lobby clearance only').trim().slice(0, 90), avatarType: type, avatarIcon: avatarIcon(type), coolPoints: num(current.coolPoints ?? localPoints()), lifetimePoints: Math.max(num(current.lifetimePoints), num(current.coolPoints ?? localPoints())), updatedAt: Date.now() }; saveLocalUsers(users); saveLocalProfileName(users[session.email].displayName); saveLocalAvatar(type); return getCurrentUser();
  }

  async function getPoints() { const user = await getCurrentUser(); return num(user?.coolPoints ?? localPoints()); }
  async function setPoints(value, reason) { const next = Math.max(0, parseInt(value, 10) || 0); const sb = await getClient(); if (sb) { const { data } = await sb.auth.getUser(); if (data && data.user) { const current = await rowFor(data.user); const lifetime = Math.max(num(current?.lifetime_points), next); saveLocalPoints(next); await sb.auth.updateUser({ data: { points: next, lifetimePoints: lifetime } }); await upsertRow(data.user, { points: next, lifetimePoints: lifetime }); return next; } } else { const session = localSession(); if (session?.email) { const users = localUsers(); if (users[session.email]) { users[session.email].coolPoints = next; users[session.email].lifetimePoints = Math.max(num(users[session.email].lifetimePoints), next); saveLocalUsers(users); } } } saveLocalPoints(next); return next; }
  async function addPoints(amount, reason) { const n = parseInt(amount, 10) || 0; const sb = await getClient(); if (sb) { const { data } = await sb.auth.getUser(); if (data && data.user) { const current = await rowFor(data.user); const currentPoints = num(current?.points ?? 0); const currentLifetime = num(current?.lifetime_points ?? currentPoints); const next = Math.max(0, currentPoints + n); saveLocalPoints(next); await upsertRow(data.user, { points: next, lifetimePoints: Math.max(currentLifetime, next) }); await addLedger(data.user, n, reason || 'site_action'); return next; } } return setPoints((await getPoints()) + n, reason); }
  async function grantVaultAccess(levelKey) { const normalizedLevel = levelKey || 'level_1'; const sb = await getClient(); if (sb) { const { data } = await sb.auth.getUser(); if (data && data.user) { await upsertRow(data.user, normalizedLevel === 'level_2' ? { level2Unlocked: true } : { level1Unlocked: true }); await addVaultUnlock(data.user, normalizedLevel); } } else { const session = localSession(); if (session?.email) { const users = localUsers(); if (users[session.email]) { if (normalizedLevel === 'level_2') users[session.email].level2Unlocked = true; else users[session.email].level1Unlocked = true; saveLocalUsers(users); } } } return true; }
  ...
  global.HWAuth = { getProviderStatus, getClient, signUpWithEmail, signInWithEmail, signInWithGoogle, signOut, getSession, getCurrentUser, updateProfile, getPoints, setPoints, addPoints, grantVaultAccess };
})(window);
