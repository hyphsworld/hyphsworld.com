...
(function () {
  ...
  function getSupabaseClient() {
    ...
    if (window.HWAuth && typeof window.HWAuth.getClient === 'function') return window.HWAuth.getClient();
    return null;
  }
  ...
  function createWidget(inline) {
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (!sb || typeof sb.rpc !== 'function') throw new Error('LOGIN_REQUIRED');
        const { data, error } = await sb.rpc('redeem_reward_code', { p_code: code });
        if (error) throw new Error(error.message || 'REDEEM_FAILED');
        if (!data || data.ok === false) throw new Error(data && data.error || 'REDEEM_FAILED');
        ...
        status.textContent = statusText(error && error.message);
      } finally {
