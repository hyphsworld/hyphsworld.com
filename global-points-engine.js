...
(function () {
  ...
  function emit(name) {
    window.dispatchEvent(new CustomEvent(name, { detail: getState() }));
  }
  ...
  function cleanLegacyGuestPoints() {
    LEGACY_KEYS.forEach((key) => {
      if (key !== 'coolPoints') localStorage.removeItem(key);
    });
  }
  ...
  async function fetchProfile(user) {
    ...
    if (window.HWAuth && window.HWAuth.supabase) {
      try {
        const id = user.userId || user.id;
        const { data } = await window.HWAuth.supabase.from('profiles').select('*').eq('id', id).maybeSingle();
        return data || null;
      } catch (error) {}
    }
    return null;
  ...
  function render() {
    ...
    document.querySelectorAll('[data-hw-points], #cool-points, #gateCredits, #wof-points').forEach((el) => {
      el.textContent = pointsText;
    });

    document.querySelectorAll('[data-hw-rank]').forEach((el) => {
      el.textContent = rank;
    });

    document.querySelectorAll('[data-hw-avatar]').forEach((el) => {
      el.textContent = avatar;
    });

    document.querySelectorAll('[data-hw-user-state]').forEach((el) => {
      el.textContent = state.user ? 'LIVE ID' : 'GUEST';
    });
  ...
  async function add(amount, reason, metadata) {
    ...
    const next = Math.max(0, number(state.points) + delta);
    state.points = next;
    state.lifetimePoints = Math.max(number(state.lifetimePoints), next);
    setLocalPoints(next);
    render();
    emit(POINT_EVENTS[1]);
    return getState();
  ...
  async function spend(amount, reason, metadata) {
    return add(-Math.abs(Number(amount || 0)), reason || 'spend', metadata || {});
  }
  ...
  window.addEventListener('storage', (event) => {
    if ([STORAGE_KEY, 'coolPoints'].includes(event.key)) refresh();
  });
})();
