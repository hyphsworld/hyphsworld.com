...
const dashboardCss = fs.readFileSync(path.join(root, 'creator-dashboard.css'), 'utf8');
...
const checks = [
  ['library UI', dashboardHtml.includes('MY CREATIONS') && dashboardHtml.includes('creationFilter') && dashboardHtml.includes('creation-grid')],
  ...
  ['visual media previews', dashboard.includes('loadCreationPreviewUrls') && dashboard.includes('createSignedUrls(paths, 3600)') && dashboard.includes('renderCreationPreview')],
  ['professional responsive gallery', dashboardCss.includes('.creation-grid') && dashboardCss.includes('.audio-wave') && dashboardCss.includes('@media(max-width:760px)')],
