document.addEventListener("DOMContentLoaded", () => {
  ...
  if (fanForm) {
    fanForm.addEventListener("submit", (e) => {
      ...
      const nameValue = fanName ? fanName.value.trim() : "";
      const emailValue = fanEmail ? fanEmail.value.trim() : "";
      ...
      if (typeof gtag === "function") {
        gtag("event", "fan_signup_attempt", {
          event_category: "fanclub",
          event_label: `${nameValue || "fan"}:${emailValue}`
        });
      }

      alert("Fan club form captured. Next step is connecting it to your email platform.");
      fanForm.reset();
    });
  }
});