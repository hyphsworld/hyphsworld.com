(function () {
  ...
  async function rewardPreview(action) {
    const client = await getSupabaseClient();
    if (!client || typeof client.rpc !== "function") return null;
    try {
      const { data, error } = await client.rpc("preview_hand_reward", { p_room_id: roomId || null, p_action: action });
      if (error) throw error;
      if (data && typeof data.balance === "number") { player.points = data.balance; setText("tableBalance", data.balance); }
      if (window.HWPoints && typeof window.HWPoints.refresh === "function") await window.HWPoints.refresh();
      return data;
    } catch (error) {
      toast(error.message || "Preview reward missed.", true);
      return null;
    }
  }
  ...
  function addToggleButton() {
    const actions = document.querySelector(".table-actions"); if (!actions || $("toggleSeatsBtn")) return;
    const btn = document.createElement("button"); btn.className = "table-btn secondary"; btn.type = "button"; btn.id = "toggleSeatsBtn"; btn.textContent = "Add More CPUs";
    btn.addEventListener("click", function () { expandedSeats = !expandedSeats; btn.textContent = expandedSeats ? "One CPU Mode" : "Add More CPUs"; if (mode === "poker" && currentHand) renderPokerTable(true); });
    actions.insertBefore(btn, actions.children[1] || null);
  }

  function renderHeader(room) { const label = modeLabel(); setText("tableTitle", label + " Table"); setText("roomMode", "Free Preview"); setText("roomCode", room?.room_code || room?.roomCode || roomCode || roomId || "Preview"); setText("previewPot", "0 CP"); setText("tableIntro", label + " room is open in free preview. No Cool Points are charged until the full gameplay loop is built."); setText("tableStatus", label + " preview loaded. Choose Start Preview to light up the felt."); setText("cpuStatus", "CPU opponent is waiting for a preview round."); setText("playerStatus", "You are seated. Buy-ins are locked for now."); }
  ...
  function renderPokerTable(keepToast) {
    const hand = currentHand || POKER_PREVIEWS[0]; const zone = $("playZone"); if (!zone) return;
    ...
    zone.innerHTML = '<div class="poker-table-shell ' + (expandedSeats ? 'is-expanded' : 'is-simple') + '"><div class="table-felt-logo">HW</div>' + seatHtml('seat-top', '🤖', 'CPU 01', 200, 'D 200', hand.cpu[0]) + extraSeats + seatHtml('seat-player', player.avatar, player.name, player.points, 'Preview', currentPlayerCards) + '<div class="community-zone"><div class="hand-badge">' + hand.label + '</div><div class="community-cards">' + hand.board.map(cardHtml).join("") + '</div><div class="pot-badge">Pot: 0 CP Preview</div></div></div>';
    if (!keepToast) toast("Poker preview dealt. +5 CP for testing the hand.", false);
  }

  async function renderPokerPreview(keepIndex) { currentHand = POKER_PREVIEWS[previewIndex % POKER_PREVIEWS.length]; currentPlayerCards = currentHand.player.slice(); renderPokerTable(false); setText("tableStatus", expandedSeats ? "Poker preview dealt with extra CPU seats. Hold, Hit, or Stay." : "Poker preview dealt one-on-one. Hold, Hit, or Stay."); setText("cpuStatus", expandedSeats ? "Multiple CPU seats active in preview mode." : "CPU 01 is active. Extra seats are optional."); setText("playerStatus", "Preview hand dealt. No wager active."); await rewardPreview("start_hand"); if (!keepIndex) previewIndex += 1; }

  function renderDominoPreview() { const tiles = DOMINO_SETS[previewIndex % DOMINO_SETS.length]; const zone = $("playZone"); if (!zone) return; zone.innerHTML = '<div class="domino-table-shell"><div class="domino-board"><div class="domino-label">CPU Tray</div><div class="domino-rack"><span class="domino-tile">▦</span><span class="domino-tile">▦</span><span class="domino-tile">▦</span></div><div class="domino-label">Center Chain</div><div class="domino-chain">' + tiles.map((tile) => '<span class="domino-tile">' + tile + '</span>').join("") + '</div><div class="domino-label">' + player.name + ' Tray</div><div class="domino-rack"><span class="domino-tile">6|6</span><span class="domino-tile">5|4</span><span class="domino-tile">3|2</span></div></div></div>'; setText("tableStatus", "Domino preview round staged. CP stays untouched until Start Round is built."); setText("cpuStatus", "CPU stacks tiles and waits."); setText("playerStatus", "Preview tray loaded. No wager active."); toast("Domino preview staged. No Cool Points charged.", false); previewIndex += 1; }

  async function handleGameAction(action) {
    if (mode === "dominoes") { toast("Hold / Hit / Stay are poker controls. Domino controls come next.", true); return; }
    if (!currentHand) await renderPokerPreview(true);
    if (action === "hit") { const card = HIT_CARDS[(currentPlayerCards.length + previewIndex) % HIT_CARDS.length]; currentPlayerCards.push(card); renderPokerTable(true); setText("tableStatus", "Hit: you drew " + card + ". +5 CP preview reward."); setText("playerStatus", "Hit accepted. Preview hand has " + currentPlayerCards.length + " cards."); await rewardPreview("hit"); toast("Hit! Drew " + card + ". +5 CP.", false); return; }
    if (action === "hold") { setText("tableStatus", "Hold: hand saved in preview mode. +5 CP reward."); setText("playerStatus", "Holding current hand."); await rewardPreview("hold"); toast("Held. +5 CP preview reward.", false); return; }
    if (action === "stay") { setText("tableStatus", "Stay: hand locked. CPU 01 reacts and checks. +5 CP reward."); setText("cpuStatus", "CPU 01 checks and waits for the next hand."); setText("playerStatus", "Stayed. Preview turn complete."); await rewardPreview("stay"); toast("Stayed. +5 CP preview reward.", false); }
  }

  function resetPreview() { currentHand = null; currentPlayerCards = []; const zone = $("playZone"); if (zone) zone.innerHTML = "<p>Choose a preview action to light up the table.</p>"; setText("tableStatus", modeLabel() + " preview reset. The room is still open."); setText("cpuStatus", "Waiting at the table."); setText("playerStatus", "Connected to the room."); toast("Preview reset.", false); }
  function startPreview() { if (mode === "dominoes") renderDominoPreview(); else renderPokerPreview(false); }

  async function loadRoom() { roomId = roomParam(); const client = await getSupabaseClient(); if (!roomId || !client) { mode = "poker"; renderHeader(null); await renderPlayer(); if (!roomId) setText("tableStatus", "No room ID found. Showing offline Poker preview."); return; } try { const { data, error } = await client.from("game_rooms").select("id,room_code,game_type,status,created_at").eq("id", roomId).maybeSingle(); if (error) throw error; mode = normalizeMode(data?.game_type); roomCode = data?.room_code || roomId; renderHeader(data); await renderPlayer(); } catch (error) { mode = "poker"; renderHeader(null); await renderPlayer(); setText("tableStatus", error.message || "Room lookup missed. Showing safe preview mode."); toast("Room lookup missed. Safe preview loaded.", true); } }
  function bind() { const year = $("year"); if (year) year.textContent = new Date().getFullYear(); const preview = $("previewDealBtn"); const reset = $("resetTableBtn"); const hold = $("holdBtn"); const hit = $("hitBtn"); const stay = $("stayBtn"); if (preview) preview.addEventListener("click", startPreview); if (reset) reset.addEventListener("click", resetPreview); if (hold) hold.addEventListener("click", function () { handleGameAction("hold"); }); if (hit) hit.addEventListener("click", function () { handleGameAction("hit"); }); if (stay) stay.addEventListener("click", function () { handleGameAction("stay"); }); addToggleButton(); }
