(function () {
  ...
  var follow = document.getElementById("followCreator"),
    share = document.getElementById("shareCreator"),
    toast = document.getElementById("worldToast"),
    audio = document.getElementById("creatorAudio"),
    play = document.getElementById("playTrack"),
    progress = document.getElementById("trackProgress"),
    shell = document.getElementById("trackProgressShell"),
    time = document.getElementById("trackTime"),
    title = document.getElementById("activeTitle"),
    meta = document.getElementById("activeMeta"),
    cover = document.getElementById("activeCover"),
    client,
    user,
    creator,
    isFollowing = false,
    busy = false,
    timer;
  if (!follow || !share || !toast || !audio || !play) return;
  ...
  async function init() {
    ...
    try {
      ...
      if (result.error || !result.data)
        throw result.error || new Error("Creator unavailable");
      creator = result.data;
      var session = await client.auth.getSession();
      user =
        session.data && session.data.session ? session.data.session.user : null;
      if (user) {
        ...
        isFollowing = Boolean(state.data);
      }
      render();
    } catch (error) {
      console.warn("KILI 631 follow state unavailable.", error);
    }
  }
  follow.addEventListener("click", async function () {
    if (busy) return;
    if (!client || !creator) {
      show("Creator connection is still loading");
      return;
    }
    if (!user) {
      show("Log in to follow creators");
      return;
    }
    busy = true;
    render();
    try {
      var response = isFollowing
        ...
        : await client
            .from("creator_follows")
            .insert({ creator_id: creator.id, user_id: user.id });
      if (response.error && response.error.code !== "23505")
        throw response.error;
      isFollowing = !isFollowing;
      show(
        isFollowing ? "You’re following Creator #010" : "Creator unfollowed",
      );
    } catch (error) {
  ...
  share.addEventListener("click", async function () {
    var data = {
      title: "KILI 631 — Creator #010",
      text: "Enter KILI 631’s Bogotá Creator World and hear MULA with Hyph Life.",
      url: location.href,
    };
    try {
      ...
      if (error && error.name !== "AbortError")
        show("Copy the profile address from your browser");
    }
  });
  play.addEventListener("click", function () {
    if (audio.paused)
      audio.play().catch(function () {
        show("Tap play again to start audio");
      });
    else audio.pause();
  });
