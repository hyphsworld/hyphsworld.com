(function () {
  ...
  function tuneFrames(root) {
    ...
    root.querySelectorAll('iframe').forEach(function (frame) {
      if (!frame.hasAttribute('loading') && !frame.closest('.homepage-full-episode-frame')) frame.loading = 'lazy';
    });
  }
  ...
  function compactPointsHud() {
    ...
    function collapse() { hud.classList.add('hwgp-compact'); hud.classList.remove('hwgp-expanded'); }
    function expand() {
    ...
    hud.addEventListener('click', function (event) {
      if (hud.classList.contains('hwgp-compact')) { event.preventDefault(); expand(); }
    });
  ...
  function idlePrefetch() {
    var run = function () {
      var targets = isGame ? ['/games.html', '/account.html'] : ['/games.html', '/creators-world.html', '/shop.html', '/account.html'];
  ...
  function ready() {
    ...
    watchDynamicUi();
    idlePrefetch();
    window.requestAnimationFrame(function () { document.documentElement.classList.add('hw-exp-ready'); });
  }
